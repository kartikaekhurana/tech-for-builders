...
const navLinks = [["services", "What We Do"], ["process", "How It Works"], ["why", "Why NivaTech"], ["contact", "Contact"]] as const;
const formFields = [["name", "Name", "text"], ["email", "Email", "email"], ["phone", "Phone / WhatsApp", "tel"], ["company", "Company / Startup", "text"], ["city", "City", "text"], ["people_count", "Number of people", "number"]] as const;
...
function Index() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("leads").insert({
      ...
      company: String(data["company"]), city: String(data["city"]), people_count: Number(data["people_count"]),
      looking_for: String(data["looking_for"]), budget: String(data["budget"]), requirements: String(data["requirements"]),
    });
    if (error) {
      setStatus("error");
      return;
  ...
  return (
    <main className="overflow-x-hidden bg-background text-foreground">
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur">
        ...
        {menuOpen && <nav className="grid border-t border-border bg-background p-5 lg:hidden">{navLinks.map(([id,label]) => <button key={id} onClick={() => jump(id)} className="border-b border-border py-4 text-left text-sm font-semibold">{label}</button>)}</nav>}
      </header>
...
function SelectField({ name, label, options, error }: { name: string; label: string; options: string[]; error?: string | undefined }) {
